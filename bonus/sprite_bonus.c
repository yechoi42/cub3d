...
/*   Updated: 2020/07/30 15:14:20 by yechoi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_sprite_config(t_game *g, t_sprite *s, double angle)
{
	double		sprite_angle;

	if (s->hit.x && s->hit.y)
	{
		set_pos(&s->mappos, (int)floor(s->hit.x / g->config.tile),
				(int)floor(s->hit.y / g->config.tile));
		set_pos(&s->mapcenter, (s->mappos.x + 0.5) * g->config.tile,
				(s->mappos.y + 0.5) * g->config.tile);
		s->content = g->config.map[(int)s->mappos.y][(int)s->mappos.x]
			== '2' ? C_S : C_TUNA;
		s->distance = hypot(g->player.x - s->mapcenter.x,
				g->player.y - s->mapcenter.y);
		sprite_angle = atan2(s->mapcenter.y - g->player.y,
				s->mapcenter.x - g->player.x);
		sprite_angle = normalize_angle(sprite_angle);
		s->min_angle = sprite_angle - atan2(g->config.tile / 2, s->distance);
		s->max_angle = sprite_angle + atan2(g->config.tile / 2, s->distance);
		...
		s->tex_x = (angle - s->min_angle) / (s->max_angle - s->min_angle)
			* g->config.tex[s->content].width - 0.0001;
	}
}
...
void	render_a_sprite(t_game *game, t_config c, t_ray *r, int rayid)
{
	...
	while (++i < r->sprite.height && ++pos[1].y < pos[0].y)
	{
		...
		pos[2].y = (pos[1].y - (pos[0].y / 2 + c.eyelevel - r->sprite.height
			/ 2)) / r->sprite.height * c.tex[r->sprite.content].height;
		color = get_tex_color(game->config.tex, r->sprite.content, &pos[2]);
